...
* [syslog](ubuntu/linux_notes?id=syslog)
 * syslog captures pretty much every log outside of authorization messages (see auth.log for that).
* _And many more_.

## syslog

`syslog` is the 'catch-all' of logs - almost all logs captured by the system are stored in the `syslog` logs (outside of authorization logs). The current log is `/var/log/syslog`, and it is a rolling log (so the current one will be re-named `syslog.1` eventually, and in turn that will eventually be re-named `syslog.2.gz` (as starting with the `.2` they are compressed). Its also possible to re-direct any log information [generated by your own service](ubuntu/linux_notes?id=creating-your-own-service) to `syslog`.
...
* The third item is the [service name](ubuntu/linux_notes?id=list-systemctl-services) that generated the message, followed by the [PID](ubuntu/linux_notes?id=process-handling) (in brackets).
* Everything after the `:` is generated by the service and thus all bets are off for formatting.

Since each service is listed in a pre-defined spot, its possible to use [journalctl](ubuntu/package_operations/journalctl) to search for messages from specific services.
...
| After | If this service should run after service XYZ, list XYZ here. It's popular to start services after [syslog](ubuntu/linux_notes?id=syslog) and networking are running (i.e. `After=syslog.target network.target`). | 
...
| StandardError | Where errors (i.e. errors that would normally print to the screen) should be sent. Its common practice to send to the [syslog](ubuntu/linux_notes?id=syslog) (i.e. `StandardError=syslog`). |
| SyslogIdentifier | How the service should be identified in the [syslog](ubuntu/linux_notes?id=syslog) (usually the same name as the service, without `.service`). |
...
* I take advantage of the fact that you can have multiple `ExecStop` - the second one prints a line similar to that found in my logging system. I do this because this will store to [syslog](ubuntu/linux_notes?id=syslog) that `systemctl stop` was called as the current service (as any messages printed in the service ExecStop will show up as the service in [syslog](ubuntu/linux_notes?id=syslog) via `journalctl`.
