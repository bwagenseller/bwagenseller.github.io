...
KVM requires a [configured Ubuntu server](operating_systems/ubuntu/server_build) and an [installed and configured KVM setup](operating_systems/kvm/kvm_install); if you need to do these things please follow these links. Also, if you would like to brush up on basic Ubuntu Linux commands [please see these instructions](operating_systems/ubuntu/linux_notes).

---  

# Launching KVM  

To launch the KVM GUI, simply type `virt-manager` on the command line.  

![virt_manager_basic.png](images/virt_manager_basic.png)

---  

# Remote Launching KVM  
...
There is an alternate way to interact with the components that need a GUI; instead of connecting to the server and then running a lightweight desktop (such as xfce), you can instead forego the server desktop and utilize [the client desktop entirely](operating_systems/kvm/kvm_notes?id=connecting-to-brand-new-vm-using-client-based-gui); note that you will need [a few packages installed on a Linux desktop](operating_systems/kvm/kvm_notes?id=client-packages-required-no-server-gui) for this, including having a [passwordless SSH key set up between your desktop client and the server](operating_systems/ubuntu/linux_notes?id=ssh). The entire install method that bypasses `virt-manager` and xfce can be found [here](operating_systems/kvm/kvm_notes?id=using-kvm-without-gui-on-server). It is _also_ possible to install a VM with absolutely **NO** GUI - [see these instructions](operating_systems/kvm/kvm_notes?id=installation-with-no-gui) on how to do that!

## Launching KVM GUI Remotely  
...
In order to run most KVM commands - in the GUI or otherwise - you must either be root or be a member of the ['libvirt'](operating_systems/kvm/kvm_install?id=adding-user-to-group-39libvirt39) group. If you have trouble doing, well, anything, check to see if you are in the 'libvirt' group.
...
There are two commands to see some info behind a pool; the first is `virsh pool-info`; you will need to get the pool name first (if you dont know it, its in [the list](operating_systems/kvm/kvm_notes?id=listing-pools), if it exists), then run `virsh pool-info NAME_OF_POOL` like so:  
```bash  
# virsh pool-info VirtualMachinesHDD
Name:           VirtualMachinesHDD
UUID:           91d6e28b-b565-4b17-8fba-c5cc171918e7
State:          running
Persistent:     yes
Autostart:      yes
Capacity:       2.73 TiB
Allocation:     85.68 GiB
Available:      2.65 TiB
```  

But this doesnt tell everything (location, etc) - to get that, run `virsh pool-dumpxml NAME_OF_POOL` like so:  
```  
# virsh pool-dumpxml VirtualMachinesHDD
<pool type='dir'>
  <name>VirtualMachinesHDD</name>
  <uuid>91d6e28b-b565-4b17-8fba-c5cc171918e7</uuid>
  <capacity unit='bytes'>3000574668800</capacity>
  <allocation unit='bytes'>92000043008</allocation>
  <available unit='bytes'>2908574625792</available>
  <source>
  </source>
  <target>
    <path>/media/backup/KVM/VirtualMachinesHDD</path>
    <permissions>
      <mode>0750</mode>
      <owner>1007</owner>
      <group>1012</group>
...
To delete a pool cleanly, you should do it in 3 steps: 

1\. Stop the pool first: `bashvirsh pool-destroy NAME_OF_POOL`  

2\. Undefine (remove) it from libvirt: `bashvirsh pool-undefine NAME_OF_POOL`  

3\. (Optional) Delete the actual directory on disk: `bashrm -rf /path/to/pool/directory`  


!> `pool-destroy` sounds scary but it just stops the pool — it does not delete your files. Your VMs/disks remain untouched. `pool-undefine` only removes the pool definition from libvirt, again leaving files intact. Only the `rm -rf` in step 3 actually deletes data from disk, so skip it if you want to keep the files.
...
If you want to save the back-end VM files to a specific location, select the 'Select or create custom image' instead and then press the 'Manage...' button. Once there, select the directory you wish to install to (remember during the setup we made a [Virtual Machine Storage pool and named it 'VirtualMachines'](operating_systems/kvm/kvm_install?id=vitrual-machine-storage)? This is that). Select the 'VirtualMachines' filesystem directory (on the left).
...
> Make sure you install `VirtIO-FS` first with `apt install virtiofsd`  

Sometimes you will need to share directories / files between the host and the VM - you can use `VirtIO-FS` to do this.  

On the host, edit the VM config via `virsh edit VM_NAME`  

Add this _before_ the `<devices>` tag:
```xml  
<memoryBacking>
  <source type='memfd'/>
  <access mode='shared'/>
...
Now, add this inside `<devices>` - this example assumes you are mounting two directories, but it can be any number:
```xml
<filesystem type='mount' accessmode='passthrough'>
  <driver type='virtiofs'/>
  <source dir='/path/on/host/share1'/>
  <target dir='share1'/>
</filesystem>

<filesystem type='mount' accessmode='passthrough'>
  <driver type='virtiofs'/>
  <source dir='/path/on/host/share2'/>
  <target dir='share2'/>
</filesystem>
```
...
```bash  
mount -t virtiofs share1 /mnt/share1
mount -t virtiofs share2 /mnt/share2
```  


To make it permanent, add to `/etc/fstab` in the guest/VM:
```
share1  /mnt/share1  virtiofs  defaults  0  0
share2  /mnt/share2  virtiofs  defaults  0  0
```  
* Make sure the mount point directories exist in the guest before rebooting  

!> _IF_ the target directory on the host contains something you mounted via `SSHFS`, this will _not_ work - virtiofs cannot share an SSHFS mount into a VM — SSHFS is a FUSE filesystem, and you can't nest FUSE inside virtiofs. Your best bet is to simply mount it in the VM itself, the same way as you do on the host.   

## VirtIO-FS: UIDs Do Not Match  

Your userID may not match between the host and VM. In order to get around this, you have have to run `id USER_NAME_HERE` on both the host and VM to see the uid; if they do not match, you can try to update them by running this in the VM (as root):   
```
usermod -u NEW_ID_HERE USER_NAME_IN_VM_HERE
groupmod -g NEW_ID_HERE USER_NAME_IN_VM_HERE
find /home/rhodesia -user OLD_ID_IN_VM_HERE -exec chown -h NEW_ID_HERE {} \;
find /home/rhodesia -group OLD_ID_IN_VM_HERE -exec chgrp -h NEW_ID_HERE {} \;
```  

You _probably_ have to enter the recovery mode in GRUB in the VM. To do so:  
1\. In virt-manager, open the VM display  

2\. Restart the VM and quickly press Shift or Esc during boot to get the GRUB menu  
...
Of note is the [UUID of the virtual machine](operating_systems/ubuntu/linux_notes?id=uuids) is displayed. Apparently it cannot be changed in the GUI, but it may be possible to edit this [in the configuration XML file](operating_systems/kvm/kvm_notes?id=xml-configuration-files).
...
You have two options: you can [select an .iso image](operating_systems/kvm/kvm_notes?id=selecting-an-iso-image) and it will appear as a 'CD/DVD' in the VM, or you can actually use _the CDROM drive on the host_ as a passthrough (so, the VM will use the host's CDROM). If you select the 'Image Location' you will have to pick an image(read: not picture)/iso file, and if you pick 'Physical Device' you will have to select the CDROM drive on your host.
...
Its possible to share a folder on the host with the VM; to do so, open the [hardware / resource settings tab](operating_systems/kvm/kvm_notes?id=editing-hardware-resource-settings) and click the 'Add Hardware' button under the list of components on the left:
...
!> Whatever host folder you pick, it MUST be owned by `libvirt-qemu:kvm` ; for me, I had to run `chmod libvirt-qemu:kvm /data/vm_shared`. Alternatively, [you can simply assign libvirt-qemu to a group and set that group owner for the host folder](operating_systems/kvm/kvm_notes?id=privileges-for-kvm).
...
The short story is each VM has at least two files: an [image file](operating_systems/kvm/kvm_install?id=image-files) that represents its hard drive (that is gigs in size) [which is stored here](operating_systems/kvm/kvm_notes?id=base-vm-directory) and an XML configuration file [which is stored here](operating_systems/kvm/kvm_notes?id=xml-configuration-files).
...
the [Virtual Machine Storage pool](operating_systems/kvm/kvm_install?id=vitrual-machine-storage) (which we named 'VirtualMachines'), you have the option of saving there instead [when you created the VM](operating_systems/kvm/kvm_notes?id=creating-a-vm-in-kvm-gui).

## Image Files

The image file is the file that acts like the 'hard drive' for the VM; its located in the [base VM directory](operating_systems/kvm/kvm_notes?id=base-vm-directory). By default, this file has an extension of .qcow2, but there are several other extensions it could be (if, for example, we imported a VM's image file from another hypervisor package like VirtualBox).
...
> Anything that you can change in [editing the VM in the GUI](operating_systems/kvm/kvm_notes?id=editing-hardware-resource-settings) you can change by editing this XML file.
...
!> This must be done from the host and **before** the VM is defined in KVM! If its already defined, you might as well [undefine](operating_systems/kvm/kvm_notes?id=remove-vm-from-kvm-manager) it right now, as KVM sets these in stone when you use `define`.
...
1\. You will now have to make a new [UUID](operating_systems/ubuntu/linux_notes?id=uuids) for the new VM. Run `uuidgen` to get a new random UUID, then edit the new VM's [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and replace the UUID with this new one.
...
```
 * Edit the new VM's [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and replace the MAC address with this new one.
...
To start a VM, either get its name from [the listing](operating_systems/kvm/kvm_notes?id=listing-the-vms) and run this:
```
...
To reboot a VM, either get its name or ID from [the listing](operating_systems/kvm/kvm_notes?id=listing-the-vms) and run either this (from the host):
```
...
To gracefully shut down a VM, either get its name or ID from [the listing](operating_systems/kvm/kvm_notes?id=listing-the-vms) and run either this (from the host):
```
...
To forcefully shut down a VM, either get its name or ID from [the listing](operating_systems/kvm/kvm_notes?id=listing-the-vms) and run either this (from the host):
```
...
The actual command is `virsh define`, but please see [the more detailed explanation](operating_systems/kvm/kvm_notes?id=adding-existing-imagexml-file-to-kvm-manager).

## Remove VM From KVM Manager

!> Careful! using `virsh undefine` on a VM will completely delete the VM's [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files)! The [image file](operating_systems/kvm/kvm_notes?id=image-files) will remain intact.

To completely remove a VM from KVM's management package, get its name from [the listing](operating_systems/kvm/kvm_notes?id=listing-the-vms) and run the following command:
```
...
!> A VM MUST [be shut down](operating_systems/kvm/kvm_notes?id=stopping-the-vm-gracefully-from-host) before it can be cloned!
...
1\. [Shut down the VM to be cloned](operating_systems/kvm/kvm_notes?id=stopping-the-vm-gracefully-from-host).

2\. Clone the VM (with the clone command above).

3\. [Start the original and new cloned VMs](operating_systems/kvm/kvm_notes?id=starting-the-vm).
...
3\. Just before you initially [start the VM](operating_systems/kvm/kvm_notes?id=starting-the-vm), set the NIC [Network source](operating_systems/kvm/kvm_notes?id=nic-networking) to "Virtual network 'default': NAT". This will ensure that this VM does not try to interface with the outside world as you change its hostname and IP.

4\. [Start the VM](operating_systems/kvm/kvm_notes?id=starting-the-vm).
...
7\. If there is a static IP to set, [set the static IP now](operating_systems/ubuntu/server_build?id=setting-up-the-static-ip).

8\. [Shut-down the VM](operating_systems/kvm/kvm_notes?id=stopping-the-vm-gracefully-from-host).

9\. Set the NIC [Network source](operating_systems/kvm/kvm_notes?id=nic-networking) of the VM to 'Specify shared device name' and enter your bridge name (most likely br0).

10\. [Re-start the VM](operating_systems/kvm/kvm_notes?id=starting-the-vm). Its now operational and ready!
...
1\. [Shut-down the VM](operating_systems/kvm/kvm_notes?id=stopping-the-vm-gracefully-from-host), 

2\. Use [guestmount](operating_systems/kvm/kvm_notes?id=libguestfs-tools) to mount the VM's filesystem (I will mount it to `/data/vm_mount` for this demonstration).
...
6\. [Re-start the VM](operating_systems/kvm/kvm_notes?id=starting-the-vm).
...
* The command that [starts a brand new VM without an OS attached](operating_systems/kvm/kvm_notes?id=starting-new-vm-with-no-os) using `virt-install`.
* The command that [utilizes a client desktop to run a GUI](operating_systems/kvm/kvm_notes?id=connecting-to-brand-new-vm-using-client-based-gui) using `virt-viewer`. This is a good option if you do not want to install even a lightweight desktop GUI on your server.
* Additional [3rd party tools](operating_systems/kvm/kvm_notes?id=additional-3rd-party-tools).

---

# Adding Existing Image/XML File to KVM Manager

If you have a VM's [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and its [image file](operating_systems/kvm/kvm_notes?id=image-files), you can add it to the KVM manager.
...
1\. [Change the unique identifiers](operating_systems/kvm/kvm_notes?id=changing-unique-identifiers).

!> If you forget to change the unique identifiers, you might as well [undefine](operating_systems/kvm/kvm_notes?id=remove-vm-from-kvm-manager) it right now, as KVM sets these in stone when you use `define`.

2\. Find the `name` tag in the [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and change it; it cannot match the name of any other VM built in KVM. The 'name' tag looks like this in the XML file:
```
<name>IAmAPirateARRRRRRRRR</name>
```

3\. Find the `source file` tag in the [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and change it to the [image file](operating_systems/kvm/kvm_notes?id=image-files) you are using (and is not associated with any running VM). The 'source file' tag looks like this in the XML file:
```
<source file='/var/lib/libvirt/images/pirate.qcow2'/>
```

4\. Now that the unique identifiers, the name, and the source file has been changed, its time to define the VM officially in KVM! If the path to your new [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) was /data/VirtualMachines/pirateparrot.xml, you would define the VM as:
```
...
6\. [Start the VM](operating_systems/kvm/kvm_notes?id=starting-the-vm).
...
10\. [Shut-down the VM](operating_systems/kvm/kvm_notes?id=stopping-the-vm-gracefully-from-host).

11\. Set the NIC [Network source](operating_systems/kvm/kvm_notes?id=nic-networking) of the VM to 'Specify shared device name' and enter your bridge name (most likely br0).

12\. [Re-start the VM](operating_systems/kvm/kvm_notes?id=starting-the-vm). Its now operational and ready!
...
1\. Find the `name` tag in the [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and make sure its correct. The 'name' tag looks like this in the XML file:
```
<name>IAmAPirateARRRRRRRRR</name>
```

2\. Find the `source file` tag in the [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) and make sure its correctly pointing to the desired [image file](operating_systems/kvm/kvm_notes?id=image-files). The 'source file' tag looks like this in the XML file:
```
<source file='/var/lib/libvirt/images/pirate.qcow2'/>
```

3\. Its time to define the VM officially in KVM! If the path to your [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files) was /data/VirtualMachines/pirateparrot.xml, you would define the VM as:
```
virsh define /data/VirtualMachines/pirateparrot.xml
```
...
1\. Get the VMs name from [the listing](operating_systems/kvm/kvm_notes?id=listing-the-vms).
...
3\. [Shut-down the VM](operating_systems/kvm/kvm_notes?id=stopping-the-vm-gracefully-from-host).

4\. Back up the [XML configuration file](operating_systems/kvm/kvm_notes?id=xml-configuration-files). If you wish, you could probably just directly copy this file; that said, you can also get it by running:
```
virsh dumpxml NAME_OF_THE_VM > /path/to/your/backup/directory/backup.MMDDYYYYHHMM.xml
```

5\. Back up the [image file](operating_systems/kvm/kvm_notes?id=image-files). You found the location of these files in step 2, so just copy them.

At this point, you can either simply back up the configuration and image files or you can [restore the VM elsewhere](operating_systems/kvm/kvm_notes?id=adding-existing-imagexml-file-to-kvm-manager).
...
Its possible to get around not having the KVM GUI; that is to say, you may not have to install `virt-manager` on the server, and you can bypass having to [launch the KVM GUI remotely](operating_systems/kvm/kvm_notes?id=launching-kvm-gui) and you can even get around [having to create a new VM in the GUI](operating_systems/kvm/kvm_notes?id=creating-a-vm-in-kvm-gui). The tradeoff is you **will** need a local desktop / machine that can run a GUI, and right now, I only know of how to do this with a Linux Desktop (I am using Ubuntu Desktop 18.04 but it can be any Linux desktop). 

## Server Packages Required (No Server GUI)

In addition to [all of these packages](operating_systems/kvm/kvm_install?id=kvm-basic-install-steps), you will also need `ssh`. On the server, [become root](/operating_systems/ubuntu/linux_notes?id=becoming-root), [update all packages](/operating_systems/ubuntu/linux_notes?id=updating-upgrading-all-packages), and then:
```
apt-get install ssh
```
...
You will also need an [SSH key](operating_systems/ubuntu/linux_notes?id=ssh) set up on the desktop to log into the remote server without a password (as the password, for whatever reason, will absolutely not work for me using `virt-viewer`)
...
3\. Download an [.iso image](operating_systems/kvm/kvm_notes?id=selecting-an-iso-image) on the server using `wget` or getting the .iso elsewhere and uploading with [scp](operating_systems/ubuntu/linux_notes?id=pushing-pulling-files-with-scp).
...
* `--disk path=/data/VirtualMachines/myshinynewvm.qcow2,size=20` defines the [image](operating_systems/kvm/kvm_notes?id=image-files) that will be used by the new VM (this cannot exist before you issue this command), as well as allocates 20 (GB) of hard disk space to the VM
...
7/. In another terminal (on your Linux desktop), [run virt-viewer to complete the installation](operating_systems/kvm/kvm_notes?id=connecting-to-brand-new-vm-using-client-based-gui).

> It seems that if you have a bridge installed and properly set-up on the server, the [NIC](operating_systems/kvm/kvm_notes?id=nic-networking) will automatically default to using the bridge.
...
> If this is part of an OS install on a brand new VM, you will have to [set up the new VM on the server first](operating_systems/kvm/kvm_notes?id=starting-new-vm-with-no-os); otherwise, these instructions will show you how to generally connect to a remote VM's display (above and beyond text).

Make sure [you have the necessary packages installed](operating_systems/kvm/kvm_notes?id=client-packages-required-no-server-gui) on your desktop, and that you have set up an [SSH key](operating_systems/ubuntu/linux_notes?id=ssh) set up on the desktop to log into the remote server without a password (as the password, for whatever reason, will absolutely not work for me using `virt-viewer`).
...
* `--cpu host` optimizes the CPU properties for the VM by exposing the host's CPU configuration to the VM.
* `--hvm` requests full hardware virtualization.
* `--disk pool=default,size=20,bus=virtio,format=qcow2` sets the pool (where the [image](operating_systems/kvm/kvm_notes?id=image-files) will be stored), allocates 20 (GB) of hard disk space to the VM, sets the bus, and sets the [image](operating_systems/kvm/kvm_notes?id=image-files) format to 'qcow2'.
...
Once its done and it has restarted, you will have to [follow these steps to enable the VM console](operating_systems/kvm/kvm_notes?id=enabling-the-vm-console).
