...
Or, alternatively:
```
apt-get install qemu-kvm libvirt-daemon-system libvirt-clients ebtables bridge-utils virt-manager virtinst uuid-runtime  -y
```

What we installed:
* qemu-kvm is KVM.
* libvirt-daemon-system and libvirt-clients are support packages for KVM.
...
After libvirtd is installed, you _may_ have to run this (as root) as per [askubuntu.com](https://askubuntu.com/questions/1225216/failed-to-connect-socket-to-var-run-libvirt-libvirt-sock) (the reason you may not is this was not required as of ~5 years ago, but it may be required now - if you skip, its easy to come back):  
```
setfacl -m user:$USER:rw /var/run/libvirt/libvirt-sock
```  
...
If you get errors that libvirtd is not running, run this (as root):  
...
Or, alternatively:
```
apt-get install libguestfs-tools python3-guestfs virt-top
```

What we installed:
* `libguestfs-tools` / `python3-guestfs` is for [libguestfs](operating_systems/kvm/kvm_notes?id=libguestfs-tools), and its used to mount image filesystems directly to the host's filesystem.
* `virt-top` is for [Virtual-top](operating_systems/kvm/kvm_notes?id=virtual-top), which is similar to htop, but for KVM.
