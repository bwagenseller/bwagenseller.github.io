...
`rsync` is a powerful tool for copying/syncing files locally or over a network. It is smart enough to skip already-copied files and resume interrupted transfers. If interrupted (Ctrl+C, network drop, etc.), just run the same command again - it picks up where it left off.  
...
* `-P` (progress + partial) - Shows progress bar AND resumes partial files if interrupted
* `-z` (compress) - Compresses during transfer (good for text, useless for media)
* `--dry-run` - Test run, shows what would happen without actually copying

**Common Use Cases**  
* Local copy: `rsync -avP /source/folder/ /destination/folder/`  
* Remote copy (push to remote): `rsync -avP /local/folder/ user@remote:/remote/folder/`  
* Remote copy (pull from remote): `rsync -avP user@remote:/remote/folder/ /local/folder/`  
* Between two remote servers:  
```
ssh user@source
rsync -avP /source/folder/ user@destination:/dest/folder/
```  

**Important: Trailing Slashes Matter!**  
* `folder/` (with slash) = copy the contents of folder (or INTO the folder, if its the target) 
* `folder` (no slash) = copy the folder itself  
...
> You may ahve to install this first: `apt-get install wget`  

The command `wget` can download a target file from the internet (provided you have the target). For example, say you wanted to download the latest version of [Asterisk](https://www.asterisk.org/) (currently version 22.3.0); its download link is `https://downloads.asterisk.org/pub/telephony/asterisk/asterisk-22-current.tar.gz`, so if we wanted to download it via wget the command would be:  
```
wget https://downloads.asterisk.org/pub/telephony/asterisk/asterisk-22-current.tar.gz
```  

> This is helpful if you use [SSH](operating_systems/ubuntu/linux_notes?id=ssh) to log into a remote box and then need to download something from the internet.  
...
| Restart | (Boolean value) If this is set to 'true', this service will re-start if the running service somehow stops via any other method outside of `systemctl stop`. | 
...
* There can also be additional shell scripts (.sh) in `/etc/profile.d` that may be run.  
  * Depending on how the .sh file is set, you may be able to source it in directly as-needed.  
...
For system-wide bash aliases that apply to all users, you have a few options depending on your distribution:  
* Put `.sh` files into `/etc/profile.d/`  
  * The `/etc/profile.d/` approach is generally preferred because it keeps custom configurations separate and makes them easier to manage or remove later.  
  * Create a custom file in `/etc/profile.d/`  
    * Name it something like `/etc/profile.d/aliases.sh`  
    * Make it executable: `chmod +x /etc/profile.d/aliases.sh`  
    * Add your aliases here  
  * This is cleaner and more maintainable since it's separate from system files  
* Alternative Approach  
  * `/etc/bash.bashrc` (Debian/Ubuntu) or `/etc/bashrc` (RHEL/CentOS/Fedora)  
    * This file is sourced for all interactive bash shells  
    * Most straightforward approach for system-wide aliases, but - you should probably use `/etc/profile.d/` instead  

> Existing users' personal aliases in ~/.bashrc or ~/.bash_aliases will override system-wide ones.  
...
To create a new group with a specific groupID: 
```
groupadd -g GROUP_ID MyGroupName
```  
...
To change the groupID, use this format:
```
...
!> The deleted group must be empty / not contain any users; to see if its empty or not, [list the users in the group](/operating_systems/ubuntu/linux_notes?id=listing-existing-groups).
...
**<font size="4">Change Ownership of Symbolic Links</font>**  

This is as per [unix.stackexchange.com](https://unix.stackexchange.com/questions/218557/how-to-change-ownership-of-symbolic-links).  

You can use `-h, --no-dereference` to change the ownership of a symbolic link.  Example:
```Ubuntu
chown -h user:group target_symlink
```  
* `target_symlink` is the symlink you are changing ownership for  
* `user:group` is the [user](operating_systems/ubuntu/linux_notes?id=adding-modifying-users) and [group](operating_systems/ubuntu/linux_notes?id=adding-modifying-groups)  
...
<font color="green">Flatpak</font> is a universal package manager for Linux; think of it as an alternative way to install apps alongside [the traditional package method](operating_systems/ubuntu/linux_notes?id=packages).  

The key differences:
* [apt](operating_systems/ubuntu/linux_notes?id=installing-a-package) (traditional)
  * Installs apps system-wide  
  * Apps share system libraries  
  * Ubuntu controls what versions are available  
  * Tightly integrated with your system  
* <font color="green">Flatpak</font>  
  * Installs apps in sandboxes (isolated containers)  
  * Each app bundles its own dependencies  
  * Apps come directly from developers (via Flathub.org)  
  * Works across all Linux distros (Ubuntu, Fedora, Arch, etc.)  
  * Apps are more isolated/secure but take up more disk space  


Think of it like this:
* [apt](operating_systems/ubuntu/linux_notes?id=installing-a-package) = traditional Linux way  
* <font color="green">Flatpak</font>  = more like how smartphone apps work (sandboxed, self-contained)
* [Snap](operating_systems/ubuntu/linux_notes?id=snap-packages) = Ubuntu's own version of the same idea  


Pros of <font color="green">Flatpak</font>:  
  * Newer versions of apps (not waiting for Ubuntu updates)  
  * Apps can't mess with your system as easily  
  * Easy to uninstall completely  

Cons:  
  * Takes more disk space (each app includes its dependencies)  
  * Slightly slower startup times  
  * Uses more RAM since libraries aren't shared  


You may ask `why not just use snap`; well...
* <font color="green">Flatpak</font> is open source  
  * Fully community-driven, not controlled by one company  
* <font color="green">Flatpak</font> is faster  
  * Noticeably quicker startup than Snap  
* <font color="green">Flatpak</font> is cleaner  
  * doesn't spam your system with loop devices  
* <font color="green">Flatpak</font> is more transparent  
  * better permission system (Flatseal lets you control app permissions easily)  
* <font color="green">Flatpak</font> is actually optional  
  * Distros don't force it on you  


## Installing Flatpak

To install <font color="green">Flatpak</font>:

1\. First, you must [become root](/operating_systems/ubuntu/linux_notes?id=becoming-root)  

2\. Update the package list with `apt-get update`  

3\. Install <font color="green">Flatpak</font>: `apt-get install flatpak`  

4\. Add the Flathub repository (Flathub is where most Flatpak apps live): `flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo`  
  * Take note of what you called the repository (in this case, `flathub`) - you _will_ have to reference this repository when installing!  

<font color="green">Flatpak</font> is now installed!  

You will have to reference the repository when installing; for example, to install Kooha, a screen capturing tool:  
```
flatpak install flathub io.github.seadve.Kooha  
```  
* `flathub` is actually referencing the repository we added earlier.  

> You may have to add repositories if what you are trying to install is not in Flathub.  


## Running a Flatpak App  

To run a <font color="green">Flatpak</font> app, you use `flatpak run`; for example, this runs Kooha, a screen capturing tool:  
```
...
We must now copy the id_ed.pub / id_rsa.pub file to the foreign server. There are two ways to do this: the esay way and the hard way.  

## Push Public Key to Remote  

The <font color="green">easy way</font> - run this from the host you just created the private/public key on: `ssh-copy-id zz9876@123.456.789.999` . This will automatically copy the prblic key up to the foreign host. 

<font color="red">Or</font>, if you want to do it old school:

1\.   To do this we will use another command available in SSH, called SCP.  This command will push the file id_ed.pub / id_rsa.pub to the foreign server from our local machine.  Run this command:
...
2\.	Use SSH to log back in to the remote server: 

```
ssh zz9876@123.456.789.999
```

3\. You are now in the foreign server.  Navigate to the .ssh folder: 

```
cd /home/zz9876/.ssh
```

4\. Rename the file id_ed.pub _or_ id_rsa.pub to "authorized_keys" (no extension): 
...
5\. Set the permissions of "authorized_keys" to 600:

```
chmod 600 /home/zz9876/.ssh/*
```

6\. Log out of the foreign server: `exit`



The setup should now be complete!  You can now test it out - run: 
...
Finally, we can search by [permissions](operating_systems/ubuntu/linux_notes?id=changing-file-permissions) with the `-perm` flag. For example, if we wanted to search for all files with permissions 755, we would do this; 
```
...
The `grep` command searches through files for keywords you specify; this can be very powerful when searching. Unlike the [find command](operating_systems/ubuntu/linux_notes?id=searching-for-files-find), `grep` actually looks through the file contents itself for words.
...
The above works for a logical `and`, but what about a logical `or`? You can use the `-e` option multiple times to achieve this. For example:  
```
grep -e "dragon" -e "kraken" /home/aa1234/*
```  
* This looks for `dragon` or `kraken`  

Using `-e` is actually using a regular expression. `Regular Expressions` are a way to match text using patterns; its very powerful but takes some getting used to. `grep` can use regular expressions if the `-E` flag is set; for more on regular expressions, [please go here](https://www.guru99.com/linux-regular-expressions.html).
...
| -z | indicates we are working with a [gzip](operating_systems/ubuntu/linux_notes?id=gzip-zipping-one-file) (a .tar.gz or .tgz) file |
...
| -p   | Shows the [process ID](/operating_systems/ubuntu/linux_notes?id=process-handling) and name of program |
...
`tcpdump` is a tool that allows you to capture network traffic on your machine; a common line I use is:  
```
tcpdump -i eth0 -s0 -w /home/user/traffic.pcap
```
* `-s` defones the snaplength (size) of the capture in bytes.  
 * It has been suggested to me to use `-s 65535` instead of `-s0`, but I just use `-s0`.  
* `-i eth0` is the interface (you can change this if you wish).  
* `-w /home/user/traffic.pcap` is the destination of the generated file.  

To stop the `tcpdump`, simply hit `Ctrl`+`c`.  

**<font size="4">Other Settings</font>**

You can also set these settings which are common:
* Port: using `tcpdump port 8080 -i eth0 -s0 -w /home/user/traffic.pcap` you could capture all traffic over port 8080.  
* Host: using `tcpdump host 10.0.0.1 -i eth0 -s0 -w /home/user/traffic.pcap` you could capture all traffic from IP 10.0.0.1.  
...
You can occasionally [check netstat](operating_systems/ubuntu/linux_notes?id=check-connections-netstat) to see your active connections - if anything looks wrong, take action.

## Monitoring auth.log

!> This is far from a proactive solution, but checking manually checking auth.log occasionally is helpful; a better solution is to use the package [fail2ban](operating_systems/ubuntu/linux_notes?id=monitoring-ssh-logins-with-fail2ban) which does this automatically.
...
While knowing where these logs are is helpful, the package [fail2ban](operating_systems/ubuntu/linux_notes?id=monitoring-ssh-logins-with-fail2ban) should be used to automatically watch ssh login attempts.
...
`fail2ban` is a package that can be loaded with `apt-get`. This package is helpful as it can guard against brute force attacks on SSH: it constantly monitors [/var/log/auth.log](operating_systems/ubuntu/linux_notes?id=monitoring-authlog) for failed login attempts; if 5 (or more) are made within a specific timeframe, the IP is banned for 5 minutes (effectively slowing brute force attacks).

To install, [become root](/operating_systems/ubuntu/linux_notes?id=becoming-root), run an `apt-get update`, and then run:
```
...
> Pass uses [GPG](operating_systems/ubuntu/linux_notes?id=gpg), so you may want to set that up and be familiar with it.  

`pass` is a simple, lightweight, standard Unix password manager that stores each password in an encrypted GPG file. Perfect for a small personal server — no daemon, no database, just git + GPG.  
...
3\. Install pass (otp = TOTP support is optional but useful): `apt-get install pass pass-extension-otp`   

4\. Initialize the store with your [GPG key](operating_systems/ubuntu/linux_notes?id=gpg): `pass init "your@email.com"`  
* _Or_ if you have multiple keys and want to specify: `pass init "Frank <your@email.com>"   
  * Or just the key ID: `pass init ABCDEF1234567890`  

After this, ~/.password-store/ is created and a .gpg-id file is written.  

### Basic Commands  

**<font size="4">List all entries (tree view)</font>**  
```bash
pass
```  

**<font size="4">Show a password (decrypts and prints)</font>**  
```bash
pass show mysql/backup-ro
```  

**<font size="4">Copy password to clipboard (works great in tmux/ssh)</font>**  
```bash
pass -c cloudflare/api-token
```  
* `-c` = copy, clears after 45s by default  

**<font size="4">Generate a strong random password (21 chars default)</font>**  
```bash
pass generate email/gmail  
pass generate --no-symbols wifi/guest 16
```  
* `pass generate email/gmail` shows it once, copies to clipboard  
* `pass generate --no-symbols wifi/guest 16` custom length, no symbols

**<font size="4">Add / edit a password interactively</font>**  
```bash
pass insert personal/nextcloud
```  
* type password (first line), Enter, then optional notes (multi-line), Ctrl+D to save

**<font size="4">Insert multiline entry (good for API tokens + comments)</font>**  
```bash
pass insert -m docker-hub/token
```  
* paste/type multiple lines, Ctrl+D to save

**<font size="4">Edit an existing entry</font>**  
```bash
pass edit servers/homeassistant/token
```  

**<font size="4">Remove an entry</font>**  
```bash
pass rm old/obsolete/key
```  

### Pass Substructure Example  
...
### Pass Script Examples  

Bash example:
```bash
DB_PASS=$(pass show mysql/backup-ro)
mysqldump -u backup --password="$DB_PASS" ... 
```  

Python example:
```python
import subprocess
password = subprocess.check_output(["pass", "show", "mysql/backup-ro"]).decode().splitlines()[0].strip()
```  
...
If you know [vi](operating_systems/ubuntu/linux_notes?id=using-vi), choose option 2; otherwise select 1 (the easiest) or 3.
...
!> Note the use of `. /home/aa1234/.profile;` ; we are sourcing in our profile here in the first command, and in the second we are issuing the actual command. If your script relies on _anything_ in your [.profle](operating_systems/ubuntu/linux_notes?id=the-profile) you **must** source in your .profile like this for each line its needed.
...
!> For our purposes of mounting filesystems, we are only concerned with things that have their [special file designator](operating_systems/ubuntu/linux_notes?id=special-file-designator) set to 'b'.
...
> You will see many `/dev/loopX` disks - ignore these, these are for [snap](operating_systems/ubuntu/linux_notes?id=snap-packages).
...
!> The main takeaway is: items that are displayed in `ls -la /dev | grep disk`, have a [special file designator](operating_systems/ubuntu/linux_notes?id=special-file-designator) set to 'b', have major = 8, and are not displayed using `fdisk -l` are harddrives / USB thumb drives / etc not mounted and are potentially mountable.

## Using lsblk

!> For more info on lsblk, [see this tutorial](https://www.howtoforge.com/linux-lsblk-command/)

Another way of looking at all filesystems is using the `lsblk` command (and I like using the -a option, which will show all filesystems, mounted and unmounted). If I run the command `lsblk -a | grep -v loop` (with the 'grep -v loop' portion to eliminate the [snap](operating_systems/ubuntu/linux_notes?id=snap-packages) entries), I get the following:
```
...
Personally, I like using `lsblk -o NAME,FSTYPE,SIZE,FSUSED,FSUSE%,MOUNTPOINTS,UUID` as it shows me some critical information.  
...
!> You must [become root](/operating_systems/ubuntu/linux_notes?id=becoming-root) for most mounting operations.
...
To see everything mounted, simply type `mount` on a terminal (since [snap](operating_systems/ubuntu/linux_notes?id=snap-packages) may have hundreds of things mounted, it may be better to remove these items from the listing by typing `mount | grep -v snap`). The results will show: device, where its mounted (on your filesystem), the type, and then settings. As an example, here is my USB thumb drive entry:
```
...
* User (uid=1002), which specifies the owner via the [uid](operating_systems/ubuntu/linux_notes?id=getting-user39s-uid-and-groups) (which in my case is user 'aa1234').
* Group (gid=1004), which specifies the group via the [gid](operating_systems/ubuntu/linux_notes?id=getting-user39s-uid-and-groups) (which in my case is group 'aa1234').

Now, lets get to mounting an unmounted partition. We found that sdd1 is an unmounted partition using [the command lsblk](operating_systems/ubuntu/linux_notes?id=using-lsblk). We will mount sdd1.
...
* We determined we can mount the partition /dev/sdd1 using [the command lsblk](operating_systems/ubuntu/linux_notes?id=using-lsblk)
* The '-o' specifies options; leave 'utf8,dmask=027,fmask=137' as is, but
 * uid is the uid of the user we want to associate with the mount; leaving this blank will associate it with root, but its better to [find the uid of the user you wish to associate this with](operating_systems/ubuntu/linux_notes?id=getting-user39s-uid-and-groups).
 * gid is the gid of the group we want to associate with the mount; leaving this blank will associate it with root, but you can [find the gid of the group you wish to associate this with](operating_systems/ubuntu/linux_notes?id=getting-user39s-uid-and-groups).
...
Unmounting is pretty straightforward; in most cases you simply need the directory you mounted to (we will use the /media/usb directory, as this is what we used in the example for [mounting](operating_systems/ubuntu/linux_notes?id=mounting-a-filesystem)):
```
umount /media/usb
```
...
If you wish to change the owner - or the location of the mount point - you will have to [unmount it](operating_systems/ubuntu/linux_notes?id=unmounting-a-filesystem) and then [re-mount it](operating_systems/ubuntu/linux_notes?id=mounting-a-filesystem) using the instructions listed.
...
> The dd command can be used for a bunch of other things outside of making a bootable usb stick from an iso - it can copy entire filesystems, etc etc. See more [here](https://www.geeksforgeeks.org/dd-command-linux/).  

You can make a bootable USB drive / thumb drive with the `dd` command (provided you downloaded the .iso file of the target operating system). You _must_ use [lsblk](operating_systems/ubuntu/linux_notes?id=using-lsblk) to find the right filesystem you wish to install the iso on (i.e. the USB thumb drive). Example:  
```
dd if=/home/someuser/Downloads/ubuntu-24.04.1.iso of=/dev/sdb1
```
* This loads `ubuntu-24.04.1.iso` to the filesystem `/dev/sdb1` (i.e. the thumb drive / usb drive).  
* Other implementations suggest using `bs=4M` but I typically leave that off - not sure if thats OK, but it seems to work.  
* Please <font color="red">be careful with this</font>, as if you pick the _wrong_ filesystem, you can end up nuking your computer.  
  * **Make sure** to have the right filesystem.  
  * You can use a few strategies to help picking the right one  
    * Using the size of the filesystem.  
    * Unmounting it, using [lsblk](operating_systems/ubuntu/linux_notes?id=using-lsblk), then re-mounting it and seeing which filesystem was added.  
...
OpenConnect is a VPN system, very similar to Cisco's AnyConnect. [This is how to install it](operating_systems/ubuntu/server_build?id=install-openconnect), but to connect simply:
...
4\. Let it sit in the terminal; unfortunately this means the window must stay open indefinitely. However, that said, you may be able to [send the process to the background](operating_systems/ubuntu/linux_notes?id=sending-running-process-to-background) which would mean you could exit that terminal.
...
You would be immediately kicked back to the prompt; if you were to [check the running processes](operating_systems/ubuntu/linux_notes?id=displaying-running-processes) you would see it running.
...
* &disown is used here, as it was done when we [used disown](operating_systems/ubuntu/linux_notes?id=using-disown) above. If you do not include `&disown` it will not run in the background.
...
The process is now in the background, so you can close the terminal. To confirm, open another terminal and [check the running processes](operating_systems/ubuntu/linux_notes?id=displaying-running-processes) to see it running.
...
To install (you must [be root](/operating_systems/ubuntu/linux_notes?id=becoming-root)):
```
...
Sometimes, [locking items to your favorites](operating_systems/ubuntu/linux_notes?id=locking-icons-to-favorites-aka-the-old-39launcher39) does not work as intended; when this happens you must manually create an executable icon. Here is how to do that.
...
Please see how to [connect to a remote desktop using Remmina](operating_systems/ubuntu/linux_notes?id=connecting-from-ubuntu-desktop).

## Renaming Bluetooth Devices  

> This is _not_ done as root!  

You cannot natively set the names of Bluetooth devices in the GUI - you must use CLI commands. To This is how I renamed my `TP-LINK_Music` to :  

1\. Type `bluetoothctl` - this will take you into the CLI for Bluetooth.  

2\. Type `devices` - it will list addresses and devices. My example:  

```Bash
Device 50:1A:A5:E2:2E:59 Jabra Speak 710
Device E4:17:D8:0D:62:12 Pro Controller
Device E4:17:D8:9A:BF:C4 Pro Controller
Device C0:25:E9:1B:33:08 TP-LINK_Music
Device FC:58:FA:CB:77:CE COWIN E7
```

3\. Find the device ID you want and connect to it - an example, using my `TP-LINK_Music` above: `connect C0:25:E9:1B:33:08`  

4\. Rename with `set-alias` (since you are connected it will rename the current device); my example: `set-alias "Computer Speakers"`  

5\. Exit: `exit`  

> Note when doing this, `bluetoothctl` may spam you with the scans it does looking for devices - you just gotta put up with it.  
