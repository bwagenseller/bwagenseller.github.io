...
In some cases, packages (usually virtualization software like [KVM](operating_systems/kvm/kvm_install) or VirtualBox) need to get their own IP from the router; `bridging` allows packages to get their own IP from the router, and it allows them to interact with the router almost as if they are their own entity / server.
...
> For more, please see [inmotionhosting](https://www.inmotionhosting.com/support/security/install-clamav-on-ubuntu/)  

ClamAV is an anti-virus for Linux. It can do basic tasks like scan for (and delete) infected files, etc etc. Its not a pro-active anti-virus - that is to say, it doesnt sit in memory watching your system. That said, it can scan files to see if there is an issue.  
...
4\. Make sure your ClamAV virus signatures are up to date.  

## Updating ClamAV Virus Signatures  

To update your ClamAV virus signatures:  

1\. [Become root](/operating_systems/ubuntu/linux_notes?id=becoming-root)  

2\. [Update all packages](/operating_systems/ubuntu/linux_notes?id=updating-upgrading-all-packages)  

3\. Stop the running process by: 
```
